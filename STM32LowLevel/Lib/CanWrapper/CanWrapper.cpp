...
bool CanWrapper::sendMessage(uint8_t msgType, const void *data, uint8_t length)
{
    ...
    if (FDCAN2->TXFQS & FDCAN_TXFQS_TFQF) return false;
    ...
    uint32_t *txAddr = reinterpret_cast<uint32_t *>(
        hfdcan2.msgRam.TxFIFOQSA + (putIdx * CANWRAP_TFQ_SIZE));
    ...
    uint32_t w0 = (static_cast<uint32_t>(padded[3]) << 24) |
                  (static_cast<uint32_t>(padded[2]) << 16) |
                  (static_cast<uint32_t>(padded[1]) <<  8) |
                   static_cast<uint32_t>(padded[0]);
    uint32_t w1 = (static_cast<uint32_t>(padded[7]) << 24) |
                  (static_cast<uint32_t>(padded[6]) << 16) |
                  (static_cast<uint32_t>(padded[5]) <<  8) |
                   static_cast<uint32_t>(padded[4]);
    txAddr[2] = w0;
    txAddr[3] = w1;
...
bool CanWrapper::readMessage(uint8_t *msgType, uint8_t *data)
{
    ...
    if ((FDCAN2->RXF0S & FDCAN_RXF0S_F0FL) == 0U) return false;
    ...
    uint32_t *rxAddr = reinterpret_cast<uint32_t *>(
        hfdcan2.msgRam.RxFIFO0SA + (getIdx * CANWRAP_RF0_SIZE));

...
{
    if (!waitClear(&I2C1->ISR, I2C_ISR_BUSY)) return false;
    ...
    if (!waitSet(&I2C1->ISR, I2C_ISR_TXIS)) return false;
    if (I2C1->ISR & I2C_ISR_NACKF) { I2C1->ICR = I2C_ICR_NACKCF; return false; }
    I2C1->TXDR = reg;
    if (!waitSet(&I2C1->ISR, I2C_ISR_TC)) return false;
    return true;
}
...
bool IMU::i2cReadBytes(uint8_t *buf, uint8_t len)
{
    ...
    for (uint8_t i = 0U; i < len; i++) {
        if (!waitSet(&I2C1->ISR, I2C_ISR_RXNE)) return false;
        if (I2C1->ISR & I2C_ISR_NACKF) { I2C1->ICR = I2C_ICR_NACKCF; return false; }
        buf[i] = static_cast<uint8_t>(I2C1->RXDR);
    }
    if (!waitSet(&I2C1->ISR, I2C_ISR_STOPF)) return false;
...
{
    if (!i2cWriteByte(reg)) return false;
...
{
    uint8_t id = 0;
    if (!readRegister(LSM6DSL_WHO_AM_I, id)) return false;
    return (id == 0x6AU);
}
...
float IMU::getPitch()  { return _cachedPitch; }
float IMU::getRoll()   { return _cachedRoll;  }

void IMU::setAlpha(float alpha) { _alpha = alpha; }
...
float IMU::getFusedPitch() { return _fusedPitch; }
float IMU::getFusedRoll()  { return _fusedRoll;  }

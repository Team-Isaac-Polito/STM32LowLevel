...
void AbsoluteEncoder::setZero()
{
    ...
    uint16_t raw = 0;
    if (!readReg16(AS5048B_ANGLMSB_REG, raw)) return;
...
{
    uint16_t raw = 0;
    if (!readReg16(AS5048B_ANGLMSB_REG, raw)) return ENCODER_READ_ERROR;
...
{
    if (!i2cWriteByte(_addr, reg)) return false;
...
bool AbsoluteEncoder::readReg16(uint8_t reg, uint16_t &out)
{
    ...
    uint8_t buf[2] = {};
    if (!i2cReadBytes(_addr, buf, 2U)) return false;

    out = static_cast<uint16_t>(static_cast<uint16_t>(buf[0]) << 6) |
          static_cast<uint16_t>(buf[1] & 0x3FU);
    return true;
}

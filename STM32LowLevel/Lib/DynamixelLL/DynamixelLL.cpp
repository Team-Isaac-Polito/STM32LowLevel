...
{
    if (numMotors < 2) {
        if (_debug) Debug.log(Level::LOG_WARN, "DXL: enableSync requires >= 2 motors\n");
        return;
    }
    disableSync();
...
{
    uint8_t packet[13] = {
        0xFF, 0xFF, 0xFD, 0x00, _servoID, 0x06, 0x00, 0x03, 0x41, 0x00, 0x00, 0x00, 0x00
    };
...
void DynamixelLL::printResponse()
{
    ...
    if (!pkt.valid)
    {
        Debug.log(Level::LOG_DEBUG, "DXL: no valid response\n");
        return;
    }
    Debug.log(Level::LOG_DEBUG, "DXL response: id=0x%02X err=0x%02X data=", pkt.id, pkt.error);
    for (uint8_t i = 0; i < pkt.dataLength; i++)
        Debug.log(Level::LOG_DEBUG, "0x%02X ", pkt.data[i]);
...
uint8_t DynamixelLL::factoryReset(uint8_t level)
{
    ...
    if (!sendPacket(packet, 11u)) return 1u;
    DxlStatusPacket rsp = receivePacket();
    return rsp.valid ? rsp.error : 1u;
}
...
{
    if (goalPosition > 4095u) goalPosition = 4095u;
...
{
    uint8_t mode = 0;
    if (torqueOnByGoalUpdate) mode |= 0x08u;
    if (timeBasedProfile)     mode |= 0x04u;
    if (reverseMode)          mode |= 0x01u;
...
uint8_t DynamixelLL::setProfileVelocity(uint32_t profileVelocity)
{
    ...
    if (profileVelocity > maxPV) profileVelocity = maxPV;
...
uint8_t DynamixelLL::setProfileAcceleration(uint32_t profileAcceleration)
{
    ...
    if (profileAcceleration > maxPA) profileAcceleration = maxPA;
    if (timeBased) {
        uint32_t pv = 0;
...
bool DynamixelLL::sendPacket(const uint8_t *packet, uint16_t length)
{
    if (_debug) {
        char msg[8];
        Debug.log(Level::LOG_DEBUG, "DXL TX(%u):", (unsigned)length);
        for (uint16_t i = 0; i < length; ++i) {
            snprintf(msg, sizeof(msg), " %02X", packet[i]);
            Debug.log(Level::LOG_DEBUG, "%s", msg);
        }
        Debug.log(Level::LOG_DEBUG, "\n");
    }
...
DxlStatusPacket DynamixelLL::receivePacket()
{
    ...
    if (!headerFound) {
        if (_debug) Debug.log(Level::LOG_WARN, "DXL: header timeout\n");
        return result;
    }
    ...
    if (idx < 7) {
        if (_debug) Debug.log(Level::LOG_WARN, "DXL: header extension timeout\n");
        return result;
    }
    ...
    if (totalPacketLength > DXL_MAX_PACKET_SIZE) {
        if (_debug) Debug.log(Level::LOG_WARN, "DXL: packet too large (%u)\n", totalPacketLength);
        return result;
    }
    ...
    if (idx < totalPacketLength) {
        if (_debug) Debug.log(Level::LOG_WARN, "DXL: incomplete packet (got %u/%u)\n", idx, totalPacketLength);
        return result;
    }

    if (_debug) {
        char msg[8];
        Debug.log(Level::LOG_DEBUG, "DXL RX(%u):", totalPacketLength);
    ...
    if (buf[7] != DXL_STATUS_INST) {
        if (_debug) Debug.log(Level::LOG_DEBUG, "DXL: echo detected, retrying\n");
    ...
    if (receivedCRC != computedCRC) {
        if (_debug) Debug.log(Level::LOG_WARN, "DXL: CRC mismatch (got %04X, expected %04X)\n",
...
uint8_t DynamixelLL::writeRegister(uint16_t address, uint32_t value, uint8_t size)
{
    if (_sync) {
        ...
        for (uint8_t i = 0; i < _numMotors; ++i) buf[i] = value;
    ...
    if (!sendPacket(packet, pktLen)) {
        if (_debug) Debug.log(Level::LOG_WARN, "DXL: write send failed\n");
        return 1u;
    }

    DxlStatusPacket rsp = receivePacket();
    if (_debug && !rsp.valid) Debug.log(Level::LOG_WARN, "DXL: write invalid response\n");
    if (_debug && rsp.error) Debug.log(Level::LOG_WARN, "DXL: write error 0x%02X\n", rsp.error);
    return rsp.error;
}

bool DynamixelLL::syncWrite(uint16_t address, uint8_t dataLength,
...
bool DynamixelLL::sendSyncWritePacket(const uint8_t *parameters, uint16_t parametersLength)
{
    ...
    packet[idx++] = 0xFD; packet[idx++] = 0x00;
...
bool DynamixelLL::sendSyncReadPacket(uint16_t address, uint8_t dataLength,
                                     const uint8_t *ids, uint8_t count)
{
    ...
    packet[idx++] = 0;
    for (uint8_t i = 0; i < count; ++i) packet[idx++] = ids[i];
    uint16_t crc = calculateCRC(packet, pktSize - 2u);
    packet[idx++] = (uint8_t)(crc & 0xFF);
    packet[idx++] = (uint8_t)(crc >> 8);
    return sendPacket(packet, pktSize);
...
uint8_t DynamixelLL::bulkRead(const uint8_t *ids, uint16_t *addresses, uint8_t *dataLengths, uint32_t *values, uint8_t count)
{
    ...
    {
        if (_debug) Debug.log(Level::LOG_WARN, "DXL: error sending Bulk Read packet\n");
        return 1;
    }
    ...
    for (uint8_t i = 0; i < count; i++)
    {
        ...
        {
            if (_debug)
                Debug.log(Level::LOG_WARN, "DXL: Bulk Read error from id %u: 0x%02X\n", ids[i], response.error);
...
bool DynamixelLL::sendBulkReadPacket(const uint8_t *ids, uint16_t *addresses,

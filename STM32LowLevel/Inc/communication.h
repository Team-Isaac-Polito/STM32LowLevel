 ...
 *
 * Differential joints (0x51/0x52):
 *   Payload: theta (yaw) and phi (pitch) as float[2].
 *   Setpoint conversion: 1a = (θ + φ)×RAD_TO_DXL, 1b = (θ − φ)×RAD_TO_DXL
 *   Feedback  inverse:   θ  = (1b − 1a) / 2, φ = −(1a + 1b) / 2  [in DXL → rad]
 * ----------------------------------------------------------------------- */
 ...
 *
 * Differential joints (0x61/0x62):
 *   Payload: theta (yaw) and phi (pitch) as float[2].
 *   Setpoint: left = (θ + φ)×RAD_TO_DXL, right = (θ − φ)×RAD_TO_DXL
 *   Feedback: θ   = (left + right) / 2,  φ = (left − right) / 2  [in DXL → rad]
 * ----------------------------------------------------------------------- */

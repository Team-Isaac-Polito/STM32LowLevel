...
__ALIGN_BEGIN static uint8_t USBD_CDC_CfgFSDesc[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END = {
    ...
    /* CMD EP */
    0x07, USB_DESC_TYPE_ENDPOINT, CDC_CMD_EP, USB_EP_TYPE_INTR,
    LOBYTE(CDC_CMD_PACKET_SIZE), HIBYTE(CDC_CMD_PACKET_SIZE), 0x0A,

    /* Data Interface */
    0x09, USB_DESC_TYPE_INTERFACE, 0x01, 0x00, 0x02, 0x0A, 0x00, 0x00, 0x00,

    /* Data OUT EP */
    0x07, USB_DESC_TYPE_ENDPOINT, CDC_OUT_EP, USB_EP_TYPE_BULK,
    LOBYTE(CDC_DATA_FS_MAX_PACKET_SIZE), HIBYTE(CDC_DATA_FS_MAX_PACKET_SIZE), 0x00,

    /* Data IN EP */
    0x07, USB_DESC_TYPE_ENDPOINT, CDC_IN_EP, USB_EP_TYPE_BULK,
    LOBYTE(CDC_DATA_FS_MAX_PACKET_SIZE), HIBYTE(CDC_DATA_FS_MAX_PACKET_SIZE), 0x00,
};
...
static uint8_t USBD_CDC_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
    ...
    {
        fops->Control(CDC_SEND_ENCAPSULATED_COMMAND, (uint8_t *)CDC_Handle.cmd,
                      CDC_Handle.dataRxLength);
    }
